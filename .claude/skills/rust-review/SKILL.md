---
name: rust-review
description: Rust code review skill.
---
...
You must not allow the use of `#[allow(clippy::too_many_lines)]`. Under no circumstances should you allow it. The implementation model tends to grant this permission arbitrarily and without authorization.

Prohibit this absolutely.

However, the following cases are permitted as exceptions:

- When the `match` arm exceeds 30 in the first place
   - Since this is physically impossible to fix, it is permitted
...
For newly created functions, use `rg -t “fn ”` to retrieve a list of functions and check whether any similar functions already exist.

In addition to coding conventions, you must pay close attention to whether file names, module names, function names, structure names, variable names, and field names are appropriate; whether the structure of files and programs is sound; and whether the flow of the code can be understood as a coherent narrative.
...
- **2nd or later review**: in **test code only**, you may relax findings that are clearly cosmetic (length, local-variable naming nits, light boilerplate, comment-style consistency). Drop those rather than file them. See `docs/coding/rust.md` §8.1.
